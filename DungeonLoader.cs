...
namespace p4gpc.dungeonloader
{
    ...
    public class DungeonLoader
    {
        ...
        /// <summary>
        /// Need to figure out a way to access the addresses, only going off of
        /// where they are now may not be viable for any potential updates.
        /// </summary>
        //private unsafe int* test = (int*)0x00AFA558;
        ...
        private DungeonTemplateFunction _templateAccessor;
        private DungeonTemplates _templates;

        private List<long> Template_Size_MovZX;

        private List<IAsmHook> _templateHooksMovZX;

        //Pointer to function that determines if a room is valid
        private IntPtr _roomLoadFuncAddress;

        //Pointer to function that determines which
        //minimap tiles are considered valid to load
        private IntPtr _minimapFuncAddress;



        public DungeonLoader(IReloadedHooks hooks, Utilities utils, IMemory memory, Config config)
        {
            _hooks = hooks;
            _utils = utils;
            _memory = memory;
            ...
            //Each json type will have an associated function class

            //Pending for deletion
            //_templateAccessor = new DungeonTemplateFunction(_jsonImporter.GetTemplates());
            _templateHooksMovZX = new List<IAsmHook>();
            //Set up garbage here, this is where we sink our hooks in
            List<Task> initialTasks = new List<Task>();
            initialTasks.Add(Task.Run((() => initDungeonTemplateLocations())));
            Task.WaitAll(initialTasks.ToArray());
        }

        private void initDungeonTemplateLocations()
        {

            List<MoveZeroExtended> moveZeroExtendeds = new List<MoveZeroExtended>();
            /*
            long test_value = _utils.SigScan("0F B6 ?? ?? E4 69 9F 00", "Template_Size_MovZX");
            _utils.Log($"Value found: {test_value}");
            */
            ...
            foreach (int address in Template_Size_MovZX)
            {
                ...
                byte testByte1;
                byte testByte2;
                _memory.SafeRead((nuint)(address + 2), out testByte1);
                _utils.LogDebug($"Byte found: {testByte1}");
                _memory.SafeRead((nuint)(address + 3), out testByte2);
                _utils.LogDebug($"Byte found: {testByte2}");
                moveZeroExtendeds.Add(new MoveZeroExtended(testByte1, testByte2, _jsonImporter.GetTemplates(), address, 0x009F69E4, _hooks, _utils));
                //moveZeroExtendeds.Last().movzx();
                //_templateHooksMovZX.Add(_hooks.CreateAsmHook(moveZeroExtendeds.Last().movzx(), address, AsmHookBehaviour.DoNotExecuteOriginal).Activate());
            }
        }
    }
