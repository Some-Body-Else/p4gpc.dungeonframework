...
namespace p4gpc.dungeonframework.Accessors
{
    public class FloorTable : Accessor
    {
        ...
        private nuint _floorDataLookupTable;
        private Int64 _floorNameTableAddress;
        ...
        protected override void Initialize()
        {
            ...
            /*
             * Does not include floor names lookup since that one is used as part of a direct value replacement, rather than through our code 
             * shennanigans. Same for the floor name table itself, since that's only accessed via the lookup address
             */
            _floorDataLookupTable = _memory.Allocate(sizeof(Int64) * 2);
            _utils.LogDebug($"New floor data lookup table address: {_floorDataLookupTable.ToString("X8")}", Config.DebugLevels.TableLocations);
            _memory.SafeWrite(_floorDataLookupTable, _newFloorTable);
            _memory.SafeWrite(_floorDataLookupTable+0x8, _chestPaletteAddress);
            ...
            _floorNameTableAddress = address;
        ...
        protected override void Update()
        {
            ...
            _chestPalletes = _jsonImporter.GetChestPalettes();


            _memory.Free(_newFloorTable);
            _memory.Free(_newFloorObjectTable);
            _memory.Free(_newFloorNames);
            _memory.Free(_newFloorNamesLookup);
            _memory.Free(_chestPaletteAddress);
            ...
            // One of the addresses is not accessed via our workaround table but is instead directly placed in memory
            _memory.SafeWrite(_floorNameTableAddress, (Int64)_newFloorNamesLookup);

            // Object table is loaded in alongside three tables from ENCOUNT.TBL
            _memory.SafeWrite(EncountTables._objectGameStartTable + 0x18, FloorTable._newFloorObjectTable);
        ...
        private void FloorTableWave1(Int64 functionAddress, string pattern, byte offsetSize)
        {
            ...
            instruction_list.Add($"mov rcx, {_floorDataLookupTable}");
        ...
        private void FloorTableWave2(Int64 functionAddress, string pattern)
        {
            ...
            instruction_list.Add($"mov rax, {_floorDataLookupTable}");
        ...
        private void FloorChestColorCheck(Int64 functionAddress, string pattern)
        {
            ...
            // RDX has palette,
            instruction_list.Add($"mov rbx, {_floorDataLookupTable}");
            instruction_list.Add($"mov rax, [rbx+0x8]");
            instruction_list.Add($"mov rbx, {_floorDataLookupTable}");
